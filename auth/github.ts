...
export default class AuthGitHub implements AuthProvider {
  ...
  async login(request: Request): Promise<Response> {
    ...
    if (requestUrl.searchParams.has("redirect_uri")) {
      try {
        referrerUrl = new URL(requestUrl.searchParams.get("redirect_uri")!);
      } catch { /* empty */ }
    }
    ...
    const baseUrl = new URL(
      requestUrl.pathname.replace(
        this.#routes.getLogin.pattern.pathname.slice(1),
        "",
      ),
      requestUrl.origin,
    );
  ...
  async logout(request: Request): Promise<Response> {
    ...
    const headers = new Headers({
      location: requestUrl.pathname.replace(
        this.#routes.postLogout.pattern.pathname.slice(1),
        "",
      ) || "/",
    });
  ...
  async getUsername(
    headers: Headers,
    _users?: Map<string, UserConfiguration>,
  ...
  async #handleGetCallback(request: Request): Promise<Response> {
    ...
    const headers = new Headers({
      location: safeRedirect ??
        (requestUrl.pathname.replace(
          this.#routes.getCallback.pattern.pathname.slice(1),
          "",
        ) || "/"),
    });
